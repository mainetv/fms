...
class BpForm6Seeder extends Seeder
{
   public function run(): void
   {
      $connection = env('DB_CONNECTION', 'mysql');
      $csvFile = database_path('seeders/csv/bp_form6.csv');
      ...
      foreach ($csv as $row) {
         DB::connection($connection)->table('bp_form6')->updateOrInsert(
            ['id' => (int) $row['id']],
            [
               'division_id' => (int) $row['division_id'],
               'year' => $row['year'],
               'tier' => isset($row['tier']) ? (int) $row['tier'] : 1,
