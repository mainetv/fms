...
class LibraryAllotmentFundSeeder extends Seeder
{
   public function run(): void
   {
      $connection = env('DB_CONNECTION', 'mysql');
      $csvFile = database_path('seeders/csv/library_allotment_funds.csv');
      ...
      foreach ($csv as $row) {
         DB::connection($connection)->table('library_allotment_funds')->insert([
            'id' => $row['id'],
            'fund_code' => $row['fund_code'] ?? null,
            'description' => $row['description'] ?? null,
            'request_status_type_id' => $row['request_status_type_id'],
            'tags' => $row['tags'] ?? null,
