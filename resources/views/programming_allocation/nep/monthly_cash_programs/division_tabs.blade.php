               ...
               <div class="col-7 col-sm-11">
                  ...
                  @foreach($divisions as $row => $value) 
                     <div role="tabpanel" @if($row == 0) class="tab-pane text-left fade in active show" 
                        ...
                        @include('programming_allocation.nep.monthly_cash_programs.division_tabs_content') 
                     </div>   
                  @endforeach   
                  </div>
               </div>
   ...
   <script type="text/javascript">     
      ...
      function changeYear()
		{         
			...
			window.location.replace("{{ url('programming_allocation/nep/monthly_cash_program/divisions') }}/"+year);
		}	
   </script>  
@endsection
