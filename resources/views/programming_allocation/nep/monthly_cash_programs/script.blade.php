...
{{-- START --}}
  ...
  {{-- add start --}}     
    function init_add_cp(){
      $('.cp-field')
        .attr('disabled', false);
        
      $('.save_cp.save-buttons')
        .addClass('d-inline')
        .removeClass('d-none')
        .attr('disabled', false);      
    }
    ...
    $('.save_cp').on('click', function(e){     
      e.prevenDefault;  
      clear_attributes();      
      $.ajax({
        method: "POST",
        url: "{{ route('monthly_cash_program.postAction') }}",
        data: {
          '_token': '{{ csrf_token() }}',      
          'add_cash_program' : 1,
  ...
  {{-- update start --}}
    function init_edit_cp(){      
      init_view_cp();
      $('.cp-field')
        .attr('disabled', false);

      $('.update_cp.save-buttons')
        .addClass('d-inline')
        .removeClass('d-none')
        .attr('disabled', false);
    }
    ...
    $('.btn_edit').on('click', function(e){  
      ...
      $.getJSON( '{{ url('programming_allocation/nep/monthly_cash_program/division/show') }}/'+id, function( data ) {
      })
      .done(function(data) {    
    ...
    $('.update_cp').on('click', function(e){   
      ...
      .then((result) => {        
        if (result.value) {          
          var request = $.ajax({
            method: "PATCH",            
            url: "{{ route('monthly_cash_program.update') }}",
            data: {
              '_token': '{{ csrf_token() }}',
              'edit_cash_program' : 1,
              'id' : $('#id').val(),
  ...
  {{-- delete start --}}
    $('#monthly_cash_program_table').on('click', '.btn_delete', function(){
      ...
      .then((result) => {
        ...
        $.ajax({
            method: "PATCH",
            url: "{{ route('monthly_cash_program.delete') }}",
            data: {
              '_token': '{{ csrf_token() }}',
              'id' : id,
  ...
  {{-- forward start --}}    
    ...
    $('.btn_forward').on('click', function(){      
      ...
      if(user_role_id == 7 && division_acronym!='COA'){ 
        ...
        alert_msg = "Are you sure you want to forward the monthly cash program to FAD-Budget?";
        notif_msg = "COA budget controller forwarded the monthly cash program for your approval or comment/s";
        ...
        if(active_status_id == 3){
          if(user_role_id == 6){
            ...
            alert_msg = "Are you sure you want to forward your divisions's monthly cash program to FAD-Budget?";     
          }
        ...
        notif_msg = "BPAC Chair has approved " + division_acronym + "'s monthly cash program.";  
    ...
    $('.forward_mcp').on('click', function(e){
      e.prevenDefault;  
      $.ajax({
        method: "POST",
        url: "{{ route('monthly_cash_program.postAction') }}",
        ...
        success:function(data) {
          ...
          if(data.success) {
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Monthly cash program has been successfully forwarded.',
                showConfirmButton: false,
                timer: 1500
            })             
            $('#forward_modal').modal('toggle') 
  ...
  {{-- receive start --}}    
    ...
    $('.btn_receive').on('click', function(){   
      ...
      if(user_role_id == 7){
        if(active_status_id == 4){     
          ...
          $(".modal-body #receive_status_id").val('17');
        }  
      ...
      else if(user_role_id==2 || user_role_id == 6 || user_role_id == 11){
        if(active_status_id == 2){ 
          ...
          else if(user_role_id == 11 || user_role_id == 2){
    ...
    $('.receive_mcp').on('click', function(e){ 
      e.prevenDefault;  
      $.ajax({
        method: "POST",
        url: "{{ route('monthly_cash_program.postAction') }}",
        ...
        success:function(data) {
          ...
          if(data.success) {
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Monthly cash program has been successfully received.',
                showConfirmButton: false,
                timer: 1500
            })             
            $('#receive_modal').modal('toggle')   
  ...
  {{-- comment start --}}    
    $('.btn_comment').on('click', function(e){     
      ...
      $.getJSON( '{{ url('programming_allocation/nep/monthly_cash_program/division/show_comment') }}/'+id, function( data ) {
      })
      .done(function(data) {    
        var i = 0;        
        ...
        while (i < rowCount) {  
          ...
          if(comment_by=='3'){
            ...
            else if(is_resolved==0 && user_role_id==3 && active_status_id==7){
          ...
          if(comment_by=='9'){   
            ...
            else if(is_resolved==0 && user_role_id==9 && active_status_id==11){              
  ...
  {{-- reverse start --}}  
    $('.btn_reverse').on('click', function(){    
      ...
      alert(user_role_id);
      if(user_role_id == 3){         
        alert_msg = "Are you sure you want to reverse the forwarding of " + division_acronym + "'s monthly cash program from BPAC?";
        notif_msg = "FAD-Budget reverse the forwarding of " + division_acronym + "'s monthly cash program from BPAC.";
    ...
    $('.reverse_mcp').on('click', function(e){
      e.prevenDefault;  
      $.ajax({
        method: "POST",
        url: "{{ route('monthly_cash_program.postAction') }}",
        ...
        success:function(data) {
          ...
          if(data.success) {
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Monthly cash program has been successfully reversed.',
                showConfirmButton: false,
                timer: 1500
            })             
            $('#reverse_modal').modal('toggle') 
