...
@section('content')
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
				<h1 class="m-0">Budget Utilization Request and Status - Local Currency (BURS-LC)</h1>
				</div>
				<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					<li class="breadcrumb-item"><a href="/fms/public">Home</a></li>
					<li class="breadcrumb-item active">Funds Utilization</li>
					<li class="breadcrumb-item active">Budget Utilization Request and Status - Local Currency (BURS-LC)</li>
				</ol>
				</div>
			</div>
		</div>
								...
								@hasanyrole('Division Budget Controller')
									<th class="text-center" style="min-width: 3%; max-width: 3%;">
										<a href="{{ url('funds_utilization/rs/add/'.$rs_type_id) }}" data-toggle="tooltip" data-placement='left'
											title='Add BURS-CFITF-AC' data-division-id="{{ $division_id }}" data-year="{{ $year_selected }}">								
...
@section('jscript')
   <script type="text/javascript">   
      $(document).ready(function(){ 
         @include('funds_utilization.burs_cfitf_ac.script')   
		...
		{         
         ...
         year = $("#year_selected").val();
			window.location.replace("{{ url('funds_utilization/rs/division/burs-cfitf-ac') }}/"+month+"/"+year);
		}
