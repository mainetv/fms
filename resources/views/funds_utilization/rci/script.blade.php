...
{{-- START --}}
  {{-- add start --}}
    ...
    $('.btn_add').on('click', function(){  
      init_add( );        
      $('#rci_modal_header').html("Add RCI");
      $('#rci_modal').modal('toggle')       
    })

    $('.add').on('click', function(e){  
      mm = $("#rci_date").datepicker('getDate').getMonth() + 1; 
      var month = ( mm < 10 ) ? ( "0" + ( mm ).toString() ) : ( mm ).toString();
      var year = $("#rci_date").datepicker('getDate').getFullYear();   
      var bank_account_id = $('#bank_account_id').val(); 
      var rci_date = $('#rci_date').val(); 
      e.prevenDefault;         
      clear_attributes();                   
      $.ajax({
        method: "POST",
        ...
        success:function(data) {
          ...
          if(data.errors) {    
            ...
            if(data.errors.rci_date){
              $('#rci_date').addClass('is-invalid');
              $('#date-error').html(data.errors.rci_date[0]);
            }           
          }
          if(data.success) {                         
            Swal.fire({
              position: 'top-end',
              icon: 'success',
              title: 'RCI has been successfully added.',
              showConfirmButton: false,
              timer: 1500
            })   
            $('#rci_modal').modal('toggle');
            $('#rci_table').DataTable().ajax.reload();
          }
        },        
      });      
    })    
  ...
  {{-- print start --}}
    $('.print').on('click', function(e){ 	
      var rci_id = $('#rci_id').val();
      window.open("{{ url('/rci/print_rci') }}/" + rci_id);
    }) 
  {{-- print end --}}  
