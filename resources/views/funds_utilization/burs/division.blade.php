...
@section('content')
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
				<h1 class="m-0">Budget Utilization Request and Status (ORS)</h1>
				</div><!-- /.col -->
				<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="/fms/public">Home</a></li>
						<li class="breadcrumb-item active">Funds Utilization</li>
						<li class="breadcrumb-item active">Budget Utilization Request and Status (ORS)</li>
				</ol>
				</div><!-- /.col -->
			</div><!-- /.row -->
		</div><!-- /.container-fluid -->
...
<th>ID</th>
								...
								<th nowrap style="min-width: 12%; max-width: 12%;">BURS No.</th>
								<th style="min-width: 24%; max-width: 24%;">Payee</th>
								<th style="min-width: 40%; max-width: 40%;">Particulars</th>
								<th nowrap style="min-width: 14%; max-width: 14%;">Amount</th>
								@role('Division Budget Controller')
									<th class="text-center" class="text-center" style="min-width: 3%; max-width: 3%;">		
										<a href="{{ url('funds_utilization/rs/add/'.$rs_type_id) }}" data-toggle="tooltip" data-placement='left'
											title='Add BURS' data-division-id="{{ $division_id }}" data-year="{{ $year_selected }}">								
      ...
      $(document).ready(function(){    
		...
		function changeFilter(){         
         ...
         year = $("#year_selected").val();
			window.location.replace("{{ url('funds_utilization/rs/division/burs') }}/"+month+"/"+year);
		}
      ...
      function getCurrenURL() {
		...
		function loadRecords(){			
			...
			var rs_table = $('#rs_table').DataTable({
				...
				responsive: true,
				ajax: {
					url: "{{ route('show_rs_by_division_month_year') }}",
					method: "GET",
					data : {
						'_token': '{{ csrf_token() }}',
						'rs_type_id' : '2',
...
{data: 'id', name: 'id', visible: false}, 
					...
					{data: null,className: 'dt-center', orderable: false,
						render: function ( data, type, full, meta ) {
							if (full.rs_no==null) {
								return '<button class="btn-xs btn_delete btn btn-outline-danger" type="button" data-id="' + full.id + '" data-toggle="tooltip" data-placement="left" title="Delete BURS"><i class="fa-solid fa-trash-can fa-lg"></i> </button>';
							} else {
