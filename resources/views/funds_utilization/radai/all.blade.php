...
@section('content')
		...
		<div class="card refreshDiv">			
			...
			<div class="card-body py-2">	
				<div class="row">
					...
					<input type="text" id="radai_id" name="radai_id" hidden>
				</div>
				
				<div class="row py-2">					
					<div class="col-4 table-responsive">
						<table id="radai_table" class="table-bordered table-hover" style="width: 100%;">
							<thead class="text-center">
								<th width="20%">RADAI No.</th>
								<th width="20%">Date</th>
								<th width="15%">Fund</th>
								<th width="30%">Bank Account No.</th>
								@role('Cash Officer')
									<th width="15%">		
										<button class='btn-xs btn_add btn btn-outline-primary' type='button' data-toggle='tooltip' 
											data-placement='left' title='Add RADAI' data-toggle="modal" data-target="#radai_modal">
					...
					<div class="col-8 table-responsive">						
						<table id="records_table" class="table-bordered table-hover" style="width: 100%;">
							<thead class="text-center">
								<th width="13%">LDDAP No.</th>
								<th width="22%">Payee</th>
								<th width="56%">Particulars</th>
	...
	@include('funds_utilization.radai.modal')	
@endsection

@section('jscript')
  <script type="text/javascript">   
		$(document).ready(function(){   
			@include('funds_utilization.radai.script')	
		...
		{         
      	...
      	year = $("#year_selected").val();
			window.location.replace("{{ url('funds_utilization/radai/all/') }}/"+month+"/"+year);
		}
		...
		$('#search_all').keyup(function() {
			radai_no = $('#radai_no').val();
			loadRecords(radai_no);
		});
		...
		function loadRADAI(){
			...
			var radai_table = $('#radai_table').DataTable({
				...
				info: false,
				ajax: {
					url: "{{ route('show_radai_by_month_year') }}",
					method: "GET",
					data : {
						'_token': '{{ csrf_token() }}',
				...
				columns: [  
					{data: 'radai_no', name: 'RAD No.', className: 'dt-center'}, 
					{data: 'radai_date', name: 'Date',  className: 'dt-center'},
		...
		$('#radai_table').on('click', '.btn_load_ada', function(e){  
			var radai_id = $(this).data('id');
			$('#radai_id').val(radai_id);
			loadRecords(radai_id);
		});

		function loadRecords(radai_id){
			...
			var records_table = $('#records_table').DataTable({
				...
				info: false,
				ajax: {
					url: "{{ route('show_ada_dvs_by_radai') }}",
					method: "GET",
					data : {
						'_token': '{{ csrf_token() }}',
						'radai_id' : radai_id,
				...
				columns: [  
					{data: 'lddap_no', name: 'ADA No.', className: 'dt-center'}, 
