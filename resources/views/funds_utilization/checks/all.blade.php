			...
			<div class="card-body py-2">			
				<div class="row py-2">
					<div class="col table-responsive">
						<table id="checks_table" class="table-bordered table-hover" style="width: 100%;">
							<thead class="text-center">
								<th nowrap style="min-width: 8d%; max-width: 8%;">Check No.</th>
								<th nowrap style="min-width: 7%; max-width: 7%;">Check Date</th>
								<th style="min-width: 23%; max-width: 23%;">Payee</th>
								...
								@role('Cash Officer')
									<th class="text-center" style="min-width: 3%; max-width: 3%;">						
										<a href="{{ url('funds_utilization/checks/add/'.date('Y-m-d')) }}" data-toggle="tooltip" data-placement='left'
											title='Add Check' data-division-id="{{ $division_id }}" data-year="{{ $year_selected }}">								
  ...
  <script type="text/javascript">   
		...
		{         
      	...
      	year = $("#year_selected").val();
			window.location.replace("{{ url('funds_utilization/checks/all/') }}/"+month+"/"+year);
		}
		...
		function loadRecords(){
			...
			var checks_table = $('#checks_table').DataTable({
				destroy: true,
				info: true,
				iDisplayLength: 50,
				scrollY: 530,
				...
				responsive: true,
				ajax: {
					url: "{{ route('show_checks_by_month_year') }}",
					method: "GET",
					data : {
						'_token': '{{ csrf_token() }}',
				...
				columns: [  
					...
					{data: 'check_date', name: 'Date',  className: 'dt-center'},
					...
					{data: 'action', defaultContent: '', name: '', className: 'dt-center', orderable: false},
				],
			});	
		}
