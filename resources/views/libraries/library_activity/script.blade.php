  ...
  {{-- view start --}}
    ...
    $('#library_activity_table').on('click', '.view-library-activity', function(e){   
      ...
      request.then(function(data) {
        if(data['status'] == 1){          
          $('#library_activity').val(data['library_activity']['activity'])   
          $('#library_activity_code').val(data['library_activity']['activity_code'])   
          $('#library_activity_description').val(data['library_activity']['description'])   
          $('#library_activity_request_status_type_id').val(data['library_activity']['request_status_type_id'])   
          $('#library_activity_remarks').val(data['library_activity']['remarks'])   
          if(data['library_activity']['is_continuing'] == 1) {
            $('#library_activity_is_is_continuing').iCheck('check'); 
          } 
  ...
  {{-- add start --}}
    ...
    $('#add_library_activity').on('click', function(e){
      ...
      $.ajax({
        ...
        data: {
        '_token': '{{ csrf_token() }}',
        'activity' : $('#library_activity').val(),
        'activity_code' : $('#library_activity_code').val(),
        'description' : $('#library_activity_description').val(),
        'request_status_type_id' : $('#library_activity_request_status_type_id').val(),
        'remarks' : $('#library_activity_remarks').val(),
        'is_continuing' :  ($('#library_activity_is_continuing').iCheck('update')[0].checked ? 1  : 0),
        ...
        success:function(data) {
          ...
          if(data.errors) {  
              if(data.errors.activity){
                $('#library_activity').addClass('is-invalid');
    ...
    $('#library_activity_table').on('click', '.update-library-activity', function(e){
      ...
      request.then(function(data) {
        if(data['status'] == 1){
          ...
          $('#library_activity_remarks').val(data['library_activity']['remarks'])   
          if(data['library_activity']['is_continuing'] == 1) {
            $('#library_activity_is_continuing').iCheck('check'); 
          }
          if(data['library_activity']['is_active'] == 1) {
            $('#library_activity_is_active').iCheck('check'); 
          } 
