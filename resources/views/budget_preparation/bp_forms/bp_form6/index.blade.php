@php
   ...
   $year = $value->year;
@endphp   
<h4>CAPITAL OUTLAY - TRANSPORTATION EQUIPMENT- Tier 1</h4>
<h6>DOST Form No. 6</h6>
<div class="content">
  <div class="row">
    <div class="col-2">
      @role('Division Budget Controller')
         <button data-division-id={{ $user_division_id }} data-year="{{ $year }}" data-fy="{{ $fiscal_year }}"
            type="button" class="btn btn-primary float-left btn_add_bp_form6" data-toggle="modal" data-target="#bp_form6_modal">Add Form 6
         </button>
      @endrole
    </div>
    <div class="col-10">
...
<div class="row py-3">
   @unlessrole('Cluster Budget Controller|Division Director|Division Budget Controller')
      @php $sqlBpForm6byFYbyTier = getBpForm6byFYbyTier($year, '1', $fiscal_year);  @endphp
   @endunlessrole
   @role('Cluster Budget Controller|Division Director')
      @php $sqlBpForm6byFYbyTier = getBpForm6byClusterbyFYbyTier($user_division_id, $year, '1', $fiscal_year);  @endphp
   @endrole
   @role('Division Budget Controller')
      @php $sqlBpForm6byFYbyTier = getBpForm6byDivisionbyFYbyTier($user_division_id, $year, '1', $fiscal_year);  @endphp
   @endrole
   <div class="col table-responsive">        
      <table id="bp_form6_table" class="table-bordered table-hover table" style="width: 100%;">
         <thead>            
            <tr>
         ...
         <tbody> <?php 
            $count = 0;
            foreach($sqlBpForm6byFYbyTier as $row){
               $id = $row->id;
               ...
               $count = count($sqlBpForm6byFYbyTier); 
               if($count != 0){?>
                  <tr>
                     ...
                     @role('Division Budget Controller')
                        <td class="text-center" style="width:6%;"> 		
                           <button type="button" class="btn-xs btn_edit_bp_form6" data-id="{{ $id }}"
                           ...
                           <button type="button" class="btn-xs btn_delete_bp_form6" data-id="{{ $id }}" 
...
<h4>CAPITAL OUTLAY - TRANSPORTATION EQUIPMENT- Tier 2</h4>
<h6>DOST Form No. 6</h6>
<div class="content">
  <div class="row">
    <div class="col-2">
...
<div class="row py-3">
   @unlessrole('Cluster Budget Controller|Division Director|Division Budget Controller')
      @php $sqlBpForm6byFYbyTier = getBpForm6byFYbyTier($year, '2', $fiscal_year);  @endphp
   @endunlessrole
   @role('Cluster Budget Controller|Division Director')
      @php $sqlBpForm6byFYbyTier = getBpForm6byClusterbyFYbyTier($user_division_id, $year, '2', $fiscal_year);  @endphp
   @endrole
   @role('Division Budget Controller')
      @php $sqlBpForm6byFYbyTier = getBpForm6byDivisionbyFYbyTier($user_division_id, $year, '2', $fiscal_year);  @endphp
   @endrole
   <div class="col table-responsive">        
      <table id="bp_form6_table" class="table-bordered table-hover table" style="width: 100%;">
         <thead>            
