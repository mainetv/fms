...
<html>
	...
	{{-- fy1--}}
		...
		<h5 class="text-center">CAPITAL OUTLAY - MACHINERIES AND EQUIPMENT</h5>
		<h6 class="text-center">(In Thousand Pesos)</h6>		
			<div class="row">
			<div class="col-10">
				<div class="float-right"><?php 
					$sqlFiscalYears = getFiscalYears($year);
					foreach($sqlFiscalYears as $row => $key){
						...
						<input type="checkbox" id="chk_fy" value="{{ $fiscal_year2; }}" 
							onclick="return false;"> FY {{ $fiscal_year2; }}<?php
					}?>
				</div>
			</div>			
		</div>	
		...
		<div class="row py-3">			
			<div class="col table-responsive">  
				<table id="bp_form5_table_fy1" class="table-bordered table-hover table" style="width: 100%;">
					<thead>
						<tr class="text-center">
							...
							<th style="min-width:100px;">Organizational Deployment</th>
							<th style="min-width:100px;">Justification</th>        
						</tr>     
					</thead>  
					<tbody>
						<?php 
							$count_fy1 = 0;			
							$sqlBpForm5byFY_fy1 = getBpForm5byDivisionbyYear($user_division_id, $year);
							foreach($sqlBpForm5byFY_fy1 as $row){
								$id = $row->id;
								...
								$count_fy1 = count($sqlBpForm5byFY_fy1);
								...
								if($count_fy1 != 0 && $fiscal_year1==$fiscal_year){?>
									<tr class="text-center">
										...
										<td class="text-left" style="max-width:300px">{{ $organizational_deployment }}</td>
							...
							$data_fy1=DB::table('bp_form5')->where('division_id',$user_division_id)->where('year',$year)							
								->where('fiscal_year', $fiscal_year1)->where('is_active', 1)->where('is_deleted', 0)->get();  
							...
							foreach($data_fy1->groupBy('year') as $key=>$row){
								...
								<tr class="text-center font-weight-bold">
									<td>TOTAL</td>
									<td>{{ $quantity_gt }}</td>	
									<td>{{ number_format($unit_cost_gt) }}</td>	
	...
	{{-- fy2--}}
		...
		<div class="row py-3">			
			<div class="col table-responsive">  
				<table id="bp_form5_table_fy2" class="table-bordered table-hover table" style="width: 100%;">
					<thead>
						<tr class="text-center">
							<th style="min-width:150px;">Item Description</th>
							<th style="min-width:55px;">Quantity</th>
					...
					<tbody>
						<?php 
							$count_fy2 = 0;			
							$sqlBpForm5byFY_fy2 = getBpForm5byDivisionbyYear($user_division_id, $year);
							foreach($sqlBpForm5byFY_fy2 as $row){
								$id = $row->id;
								$description = $row->description;
								$quantity = $row->quantity;
								$unit_cost = $row->unit_cost;
								...
								$remarks = $row->remarks;
								$is_active = $row->is_active;
								$count_fy2 = count($sqlBpForm5byFY_fy2);
								$unit_cost = round($row->unit_cost, -3)/1000;
								$total_cost = round($row->total_cost, -3)/1000;
								$fiscal_year = $row->fiscal_year;
							...
							$data_fy2=DB::table('bp_form5')->where('division_id',$user_division_id)->where('year',$year)
								->where('fiscal_year', $fiscal_year2)->where('is_active', 1)->where('is_deleted', 0)->get();  
	...
	{{-- fy3--}}
				...
				<div class="float-right"><?php 
					...
					foreach($sqlFiscalYears as $row => $key){
						...
						<input type="checkbox" id="chk_fy" value="{{ $fiscal_year2; }}" 
							onclick="return false;"> FY {{ $fiscal_year2; }}<br>
						<input type="checkbox" id="chk_fy" value="{{ $fiscal_year2; }}" 
							@if($fiscal_year1) checked @endif onclick="return false;"> FY {{ $fiscal_year2; }}<?php
					}?>
				</div>
			</div>			
		</div>	
		...
		<div class="row py-3">			
			<div class="col table-responsive">  
				<table id="bp_form5_table_fy3" class="table-bordered table-hover table" style="width: 100%;">
					<thead>
						<tr class="text-center">
							<th style="min-width:150px;">Item Description</th>
							<th style="min-width:55px;">Quantity</th>
					...
					<tbody>
						<?php 
							$count_fy3 = 0;			
							$sqlBpForm5byFY_fy3 = getBpForm5byDivisionbyYear($user_division_id, $year);
							foreach($sqlBpForm5byFY_fy3 as $row){
								$id = $row->id;
								$description = $row->description;
								$quantity = $row->quantity;
								$unit_cost = $row->unit_cost;
								...
								$remarks = $row->remarks;
								$is_active = $row->is_active;
								$count = count($sqlBpForm5byFY_fy3);
								$unit_cost = round($row->unit_cost, -3)/1000;
								$total_cost = round($row->total_cost, -3)/1000;
								$fiscal_year = $row->fiscal_year;
							...
							$data_fy3=DB::table('bp_form5')->where('division_id',$user_division_id)->where('year',$year)
								->where('fiscal_year', $fiscal_year3)->where('is_active', 1)->where('is_deleted', 0)->get();  
