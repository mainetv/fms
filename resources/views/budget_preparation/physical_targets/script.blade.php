      ...
      request.then(function(data) {
        if(data['status'] == 1){   
          ...
          view_object_expenditure_by_expense_account_id(
            data['budget_proposal']['expense_account_id'],
          ...
          while (i < rowCount) {  
            ...
            if(comment_by=='Division Director'){  
              if(is_resolved==1){
                ...
                tdata += '<td><input type="hidden" name="by_director_is_resolved[]" id="by_director_is_resolved" value="' + is_resolved + '"></td>';
                tdata += '<td><button type="button" data-toggle="tooltip" data-placement="auto" title="Delete Comment" @if($user_role_id==6 || $user_role_id==11) class="remove-row btn btn-danger btn-sm" @else class="d-none" @endif><i class="fa-solid fa-xmark"></i></button></td></tr>';
                $(".director_comments_table").append(tdata);
                i++;
              }
            }
        ...
        if(data['status'] == 1){   
          ...
          if(user_role_id == 3){            
            if(active_status_id == 7){
              notif_msg = "FAD-Budget forwarded " + division_acro + "'s budget proposal to BPAC.";
              msg = "Are you sure you want to forward  " + division_acro + "'s budget proposal to BPAC?";
              $(".modal-body #forward_division_id_to").val('9');
              $(".modal-body #forward_user_role_id_to").val('9');
