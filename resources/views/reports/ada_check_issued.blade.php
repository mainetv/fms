	...
	<section class="content">
		<div class="card text-left">
			...
			<div class="card-body">
				<div class="row">
					<div> 												
						<select name="payment_mode_selected" id="payment_mode_selected" class="select_filter">
							<option value="All">All</option>
							@foreach($getPaymentModes as $row)
								<option value="{{ $row->id }}" data-selected="{{ $row->id }}">{{ $row->payment_mode }}</option>
							@endforeach
						</select>
					</div>&emsp;
					<div>
	...
	<script type="text/javascript">   
		...
		$('#date_range').daterangepicker(
			...
			function(start, end, label) {
				...
				loadRecords(payment_mode_selected, fund_id_selected, start_date, end_date);
			},
		);
		...
		var payment_mode_selected=$('#payment_mode_selected').val();	
		...
		function loadRecords(payment_mode_selected, fund_id_selected, start_date, end_date){	
			var records_table = $('#records_table').DataTable({				
				...
				columns: [
					{data: 'ada_check_date', title: 'Date', width: '5%', className: 'dt-head-center'},  
					{data: 'ada_check_no', title: 'ADA/Check No.', width: '6%', className: 'dt-center'},
					...
					{data: 'liquidated_damages', title: 'Liq. Damages', width: '3%', className: 'dt-head-center dt-body-right',
						render: $.fn.dataTable.render.number(',', '.', 2, '')
					},
					{data: 'other_deductions', title: 'Other', width: '3%', className: 'dt-head-center dt-body-right',
						render: $.fn.dataTable.render.number(',', '.', 2, '')
					},
					{data: 'total_dv_net_amount', title: 'Net Amount', width: '7%', className: 'dt-head-center dt-body-right',
						render: $.fn.dataTable.render.number(',', '.', 2, '')
					},
				]
			});			
