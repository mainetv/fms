...
$data_sum = DB::table('view_lddap_check_tax')->whereMonth('date', $month_selected)->whereYear('date', $year_selected)
										->where('is_active', 1)->where('is_deleted', 0)->where('total_tax','>',0)->sum('total_tax');
										foreach($data as $row){?>
										<tr class="text-right">
											<td class="text-center">{{ $row->date }}</td>
											<td class="text-left">{{ $row->payee }}</td>
											<td class="text-center">{{ $row->tin }}</td>
											<td class="text-center">{{ $row->dv_no }}</td>
											<td class="text-center">{{ $row->ref_no }}</td>
											<td>{{ number_format($row->tax_one,2) }}</td>
											<td>{{ number_format($row->tax_two,2) }}</td>
											<td>{{ number_format($row->tax_twob,2) }}</td>
											<td>{{ number_format($row->tax_three,2) }}</td>
											<td>{{ number_format($row->tax_five,2) }}</td>
											<td>{{ number_format($row->tax_six,2) }}</td>
...
<tr class="font-weight-bold text-right">
											...
											<td>{{ number_format($data_sum,2) }}</td>
										</tr>										
								</tbody>
							</table>	 
						</div>    
...
@section('jscript')
	<script type="text/javascript">   
		...
		$('.select_filter').on('change', function(){			
			...
			window.location.replace("{{ url('reports/monthly_wtax') }}/"+month_selected+"/"+year_selected);
		})	
	</script> 
@endsection	