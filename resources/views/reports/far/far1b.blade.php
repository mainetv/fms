			...
			<div class="card-header row">
				<div class="col-10">
					<h5 class="font-weight-bold">
						Statement of Appropiations, Allotments, Obligations, Disbursements and Balances (FAR 1B)
					</h5> 
				</div>
						...
						<div class="col table-responsive">
							<table width="100%" class="table-hover table2excel" id="tbl_far1" style="font-size: 11px;">
								<thead class="text-center">
									<tr>
										<th rowspan="2">No.</th>
										<th colspan="2">Allotments/Sub-Allotments Reference</th>
										<th colspan="2">Funding Source</th>										
										<th colspan="5">Allotments/Sub-Allotments Reference received from CO/ROs/OUs</th>
										<th colspan="5">Sub-Allotment to ROs/OUs</th>
										<th colspan="5">Total Allotments/Net of Sub-Allotments</th>
									</tr>
									<tr>
										<th>Number</th>
										<th>Date</th>																				
								...
								<tbody>
									...
									@endphp	
									<tr>
										<td colspan="2" nowrap>A. Allotments received from DBM</td>
										<td></td>		
										<td></td>
										<td></td>
										<td></td>				
									...
									@foreach($data->groupBy('allotment_fund_reference_id') as $key=>$row)										
										...
										<tr class="text-right">
											...
											<td class="text-left">{{ $item->allotment_received_dbm }}</td>	
											<td class="text-center">{{ $item->allotment_date_received }}</td>	
											<td class="text-left">{{ $item->fund_description }}</td>	
											<td class="text-center">{{ $item->fund_uacs_code }}</td>												
											<td>{{ number_format($total_allotment_adjustment,2) }}</td>												
											<td>{{ number_format($total_allotment_adjustment,2) }}</td>														
											<td>{{ number_format($total_allotment_adjustment,2) }}</td>														
											<td>{{ number_format($total_allotment_adjustment,2) }}</td>														
											<td>{{ number_format($total_allotment_adjustment,2) }}</td>																
