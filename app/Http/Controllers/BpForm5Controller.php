...
class BpForm5Controller extends Controller
{
    /**
    ...
    public function index($year_selected)
    {
        ...
        $years = FiscalYearsModel::orderBy('year', 'DESC')->get();
        $data = [
        ...
        $bp_form5 = BpForm5Model::where('year', $year_selected)->where('is_active', 1)->where('is_deleted', 0)->get();

        return view('budget_preparation.bp_forms.bp_form5.fy_tabs')
            ...
            ->with(compact('bp_form5'), $bp_form5);
    }
    ...
    public function generatePDF(Request $request, $division_id, $year)
    {
        if ($request->ajax()) {
            ...
            return view('budget_preparation.bp_forms.bp_form5.pdf')
     ...
     *
     * @param  \App\Models\BpForm5Model  $BpForm5Model
     * @return \Illuminate\Http\Response
     */
    public function show(Request $request)
    {
        if ($request->ajax()) {
            $data = BpForm5Model::find($request->get('id'));
            if ($data->count()) {
    ...
    public function view(Request $request)
    {
        // if($request->ajax()){
        $user_id = auth()->user()->id;
        $data = ViewUsersModel::where('id', $user_id)->pluck('division_id')->first();
        if ($data->count()) {
            return Response::json([
                'status' => '1',
                'user_division' => $data,
    ...
    public function update(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($validator->passes()) {
                // dd($request->all());
                BpForm5Model::find($request->get('id'))
                    ->update([
                        'tier' => $request->get('tier'),
                        'description' => $request->get('description'),
                        'quantity' => $request->get('quantity'),
     ...
     * @return \Illuminate\Http\Response
     */
    public function delete(Request $request, BpForm5Model $BpForm5Model)
    {
        if ($request->ajax()) {
            try {
                BpForm5Model::find($request->get('id'))
