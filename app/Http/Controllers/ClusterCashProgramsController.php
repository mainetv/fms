...
class ClusterCashProgramsController extends Controller
{
    use HasRoles;
    ...
    public function index($year_selected)
    {
        ...
        if (isset(request()->url)) {
            ...
            if (auth()->user()->hasAnyRole('Cluster Budget Controller')) {
                return view('programming_allocation.nep.monthly_cash_programs.cluster')
                    ->with(compact('title'))
                ...
                return view('programming_allocation.nep.monthly_cash_programs.cluster_tabs')
                    ->with(compact('title'))
                    ->with(compact('user_id'))
    ...
    public function generatePDF(Request $request, $division_id, $year)
    {
        if ($request->ajax()) {
            ...
            $pdf->loadView('cash_programs.cluster_print')
                ->set_paper('letter', 'landscape');

            return $pdf->stream();
        } else {
            return view('programming_allocation.nep.monthly_cash_programs.cluster_print')
