...
class LDDAPController extends Controller
{
      ...
      if ($request->ajax()) { 
         ...
         if($request->add_lddap==1){
            ...
            if ($validator->passes()) {
               ...
               $data->save(); 
               $data->id;    
               return response()->json(array('success' => 1,'redirect_url' => route('lddap.edit', [$data->id]),), 200); 
            }
   ...
   public function update(Request $request){
      if($request->ajax()) { 
         ...
         else if($request->attach_update_dv==1){  
				if(isset($dv_id)){   
               // dd($request->all());
					DVModel::find($dv_id)
						->update([ 
         ...
         else if($request->replace_payee_bank_account==1){  
				if(isset($dv_id)){   
					DVModel::find($dv_id)
						->update([ 
							'payee_id'=>$payee_id,
