...
class ADAController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            if ($request->add_ada == 1) {
                ...
                if ($validator->passes()) {
                    ...
                    return response()->json(['success' => 1, 'redirect_url' => route('ada.edit', [$data->id])], 200);
                }
    ...
    public function update(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($request->edit_ada == 1) {
                ...
                return Response::json(['errors' => $validator->errors()]);
            } elseif ($request->attach_update_lddap == 1) {
                // dd($request->all());
                if (isset($lddap_id)) {
    ...
    public function show_ada_by_fund_month_year(Request $request)
    {
        ...
        if ($request->ajax() && isset($month_selected) && $month_selected != null && ($search == null || $search == '')) {
            ...
            $data = ViewADAModel::where(function ($query) use ($search) {
